...
$app->map(['GET','POST'],'/getAssets', function (Request $request, Response $response){
    ...
    $ASSET_SUB_LOCATION = strtoupper($data->sub_location);
    $ASSET_PRIMAY_ID = strtoupper($data->asset_primary_id);
    $ASSET_DESCRIPTION = strtoupper($data->description);
    $ASSET_CLASS = strtoupper($data->asset_class);
    
    $response = array();

    if(!empty($building) || !empty($level) || !empty($area) || !empty($ASSET_DESCRIPTION) || !empty($ASSET_CLASS)  || !empty($ASSET_SUB_LOCATION) || !empty($ASSET_PRIMAY_ID)){
        ...
        AND ASSET_SUB_LOCATION LIKE '%$ASSET_SUB_LOCATION%' 
        AND ASSET_CLASS LIKE '%$ASSET_CLASS%' 
        AND ASSET_PRIMARY_ID LIKE '%$ASSET_PRIMAY_ID%' 
